...
Public Sub testImport(projName As String, importPath As String)
    'Dim proj_name As String
    'proj_name = "SheetInitialization"
...
Public Sub testExport(projName As String, exportPath As String)
    'Dim proj_name As String
    'proj_name = "SheetInitialization"

    Dim vbaProject As Object
    Set vbaProject = Application.VBE.VBProjects(projName)
    Build.exportVbaCode vbaProject:=vbaProject, exportPath:=exportPath
End Sub
...
Public Function getSourceDir(importPath As String, createIfNotExists As Boolean) As String
    ' First check if the fullWorkbookPath contains a \.
    If Not InStr(importPath, "\") > 0 Then
...
Public Sub exportVbaCode(vbaProject As VBProject, exportPath As String)
...
Public Sub importVbaCode(vbaProject As VBProject, importPath As String, Optional includeClassFiles As Boolean = False)
    Dim vbProjectFileName As String
    On Error Resume Next
    'this can throw if the workbook has never been saved.
    vbProjectFileName = vbaProject.fileName
    ...
    Dim export_path As String
    export_path = getSourceDir(importPath, createIfNotExists:=False)
    If export_path = "" Then
